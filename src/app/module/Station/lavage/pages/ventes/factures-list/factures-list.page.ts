...
@Component({
  selector: 'app-lavage-factures-list-page',
  standalone: true,
...
export class FacturesListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(LavageActions.loadFactures());
  }
  ...
  onSaved(draft: FactureDraft): void {
    ...
    if (editing) {
      this.store.dispatch(LavageActions.updateFacture({ id: editing.id, draft }));
    } else {
      this.store.dispatch(LavageActions.addFacture({ draft }));
    }
  ...
  removeFacture(id: number): void {
    if (confirm('Supprimer cette facture ?')) {
      this.store.dispatch(LavageActions.removeFacture({ id }));
    }
  }
