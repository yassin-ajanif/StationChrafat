...
@Component({
  selector: 'app-lavage-achat-livraisons-list-page',
  standalone: true,
...
export class AchatLivraisonsListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(LavageActions.loadReceptions());
  }
  ...
  onSaved(draft: ReceptionDraft): void {
    ...
    if (editing) {
      this.store.dispatch(LavageActions.updateReception({ id: editing.id, draft }));
    } else {
      this.store.dispatch(LavageActions.addReception({ draft }));
    }
  ...
  removeReception(id: number): void {
    if (confirm('Supprimer cette réception ?')) {
      this.store.dispatch(LavageActions.removeReception({ id }));
    }
  }
