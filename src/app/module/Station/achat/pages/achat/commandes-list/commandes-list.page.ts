...
export class AchatCommandesListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(AchatActions.loadCommandesAchat());
  }
  ...
  onSaved(draft: CommandeAchatDraft): void {
    ...
    if (editing) {
      this.store.dispatch(AchatActions.updateCommandeAchat({ id: editing.id, draft }));
    } else {
      this.store.dispatch(AchatActions.addCommandeAchat({ draft }));
    }
