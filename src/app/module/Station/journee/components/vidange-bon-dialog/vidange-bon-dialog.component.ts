...
import {
  ...
  VIDANGE_CONSUMABLE_PRODUCTS,
  VIDANGE_SERVICE_TYPES,
  VidangeBon,
...
@Component({
  selector: 'app-vidange-bon-dialog',
  standalone: true,
  imports: [ButtonComponent, DecimalPipe],
  templateUrl: './vidange-bon-dialog.component.html',
  styleUrl: './vidange-bon-dialog.component.scss',
})
export class VidangeBonDialogComponent {
  ...
  readonly serviceTypes = VIDANGE_SERVICE_TYPES;
  readonly consumableProducts = VIDANGE_CONSUMABLE_PRODUCTS;
  ...
  private createEmptyServiceRow(): ServiceTableRow {
    return { id: ++this.serviceSeq, serviceType: '', amount: null };
  }
  ...
  updateServiceType(rowId: number, event: Event): void {
    const serviceType = (event.target as HTMLSelectElement).value;
    this.patchServiceRow(rowId, { serviceType });
  }
  ...
  clearServiceRow(rowId: number): void {
    this.patchServiceRow(rowId, { serviceType: '', amount: null });
  }
