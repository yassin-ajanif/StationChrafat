...
export class AchatAvoirsListPage implements OnInit {
  ...
  onSaved(draft: AvoirFournisseurDraft): void {
    ...
    if (editing) {
      this.store.dispatch(CarburantActions.updateAvoirFournisseur({ id: editing.id, draft }));
    } else {
      this.store.dispatch(CarburantActions.addAvoirFournisseur({ draft }));
    }
  ...
  removeAvoir(id: number): void {
    if (confirm('Supprimer cet avoir fournisseur ?')) {
      this.store.dispatch(CarburantActions.removeAvoirFournisseur({ id }));
    }
  }
