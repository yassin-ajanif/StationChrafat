...
export class AchatLivraisonsListPage implements OnInit {
  ...
  onSaved(draft: ReceptionDraft): void {
    ...
    if (editing) {
      this.store.dispatch(CarburantActions.updateReception({ id: editing.id, draft }));
    } else {
      this.store.dispatch(CarburantActions.addReception({ draft }));
    }
  ...
  removeReception(id: number): void {
    if (confirm('Supprimer ce bon de livraison ?')) {
      this.store.dispatch(CarburantActions.removeReception({ id }));
    }
  }
