...
export class AchatDevisListPage implements OnInit {
  ...
  onSaved(draft: DevisAchatDraft): void {
    ...
    if (editing) {
      this.store.dispatch(CarburantActions.updateDevisAchat({ id: editing.id, draft }));
    } else {
      this.store.dispatch(CarburantActions.addDevisAchat({ draft }));
    }
  ...
  removeDevis(id: number): void {
    if (confirm('Supprimer ce devis fournisseur ?')) {
      this.store.dispatch(CarburantActions.removeDevisAchat({ id }));
    }
  }
