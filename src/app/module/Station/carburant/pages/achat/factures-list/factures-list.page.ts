...
export class AchatFacturesListPage implements OnInit {
  ...
  onSaved(draft: FactureFournisseurDraft): void {
    ...
    if (editing) {
      this.store.dispatch(CarburantActions.updateFactureFournisseur({ id: editing.id, draft }));
    } else {
      this.store.dispatch(CarburantActions.addFactureFournisseur({ draft }));
    }
  ...
  removeFacture(id: number): void {
    if (confirm('Supprimer cette facture fournisseur ?')) {
      this.store.dispatch(CarburantActions.removeFactureFournisseur({ id }));
    }
  }
