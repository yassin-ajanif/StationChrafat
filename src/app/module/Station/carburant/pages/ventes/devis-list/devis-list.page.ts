...
@Component({
  selector: 'app-carburant-devis-list-page',
  standalone: true,
...
export class DevisListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(CarburantActions.loadDevis());
  }
  ...
  onSaved(draft: DevisDraft): void {
    ...
    if (editing) {
      this.store.dispatch(CarburantActions.updateDevis({ id: editing.id, draft }));
    } else {
      this.store.dispatch(CarburantActions.addDevis({ draft }));
    }
  ...
  removeDevis(id: number): void {
    if (confirm('Supprimer ce devis ?')) {
      this.store.dispatch(CarburantActions.removeDevis({ id }));
    }
  }
