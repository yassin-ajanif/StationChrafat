...
export class LivraisonsListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(ErpActions.loadLivraisons());
  }
  ...
  onSaved(draft: LivraisonDraft): void {
    ...
    if (editing) {
      this.store.dispatch(ErpActions.updateLivraison({ id: editing.id, draft }));
    } else {
      this.store.dispatch(ErpActions.addLivraison({ draft }));
    }
  ...
  removeLivraison(id: number): void {
    if (confirm('Supprimer cette livraison ?')) {
      this.store.dispatch(ErpActions.removeLivraison({ id }));
    }
  }
