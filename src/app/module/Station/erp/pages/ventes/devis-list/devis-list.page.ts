...
export class DevisListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(ErpActions.loadDevis());
  }
  ...
  onSaved(draft: DevisDraft): void {
    ...
    if (editing) {
      this.store.dispatch(ErpActions.updateDevis({ id: editing.id, draft }));
    } else {
      this.store.dispatch(ErpActions.addDevis({ draft }));
    }
  ...
  removeDevis(id: number): void {
    if (confirm('Supprimer ce devis ?')) {
      this.store.dispatch(ErpActions.removeDevis({ id }));
    }
  }
