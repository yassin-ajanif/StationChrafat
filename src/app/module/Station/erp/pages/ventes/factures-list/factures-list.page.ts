...
export class FacturesListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(ErpActions.loadFactures());
  }
  ...
  onSaved(draft: FactureDraft): void {
    ...
    if (editing) {
      this.store.dispatch(ErpActions.updateFacture({ id: editing.id, draft }));
    } else {
      this.store.dispatch(ErpActions.addFacture({ draft }));
    }
  ...
  removeFacture(id: number): void {
    if (confirm('Supprimer cette facture ?')) {
      this.store.dispatch(ErpActions.removeFacture({ id }));
    }
  }
