...
export class AvoirsListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(ErpActions.loadAvoirs());
  }
  ...
  onSaved(draft: AvoirDraft): void {
    ...
    if (editing) {
      this.store.dispatch(ErpActions.updateAvoir({ id: editing.id, draft }));
    } else {
      this.store.dispatch(ErpActions.addAvoir({ draft }));
    }
  ...
  removeAvoir(id: number): void {
    if (confirm('Supprimer cet avoir ?')) {
      this.store.dispatch(ErpActions.removeAvoir({ id }));
    }
  }
