...
export class RetoursListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(ErpActions.loadRetoursFournisseur());
  }
  ...
  onSaved(draft: RetourFournisseurDraft): void {
    ...
    if (editing) {
      this.store.dispatch(ErpActions.updateRetourFournisseur({ id: editing.id, draft }));
    } else {
      this.store.dispatch(ErpActions.addRetourFournisseur({ draft }));
    }
  ...
  removeRetour(id: number): void {
    if (confirm('Supprimer ce bon de retour ?')) {
      this.store.dispatch(ErpActions.removeRetourFournisseur({ id }));
    }
  }
