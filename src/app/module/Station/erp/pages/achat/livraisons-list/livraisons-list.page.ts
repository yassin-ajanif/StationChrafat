...
export class AchatLivraisonsListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(ErpActions.loadReceptions());
  }
  ...
  onSaved(draft: ReceptionDraft): void {
    ...
    if (editing) {
      this.store.dispatch(ErpActions.updateReception({ id: editing.id, draft }));
    } else {
      this.store.dispatch(ErpActions.addReception({ draft }));
    }
  ...
  removeReception(id: number): void {
    if (confirm('Supprimer cette réception ?')) {
      this.store.dispatch(ErpActions.removeReception({ id }));
    }
  }
