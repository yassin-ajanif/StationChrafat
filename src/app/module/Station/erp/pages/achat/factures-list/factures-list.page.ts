...
export class AchatFacturesListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(ErpActions.loadFacturesFournisseur());
  }
  ...
  onSaved(draft: FactureFournisseurDraft): void {
    ...
    if (editing) {
      this.store.dispatch(ErpActions.updateFactureFournisseur({ id: editing.id, draft }));
    } else {
      this.store.dispatch(ErpActions.addFactureFournisseur({ draft }));
    }
  ...
  removeFacture(id: number): void {
    if (confirm('Supprimer cette facture fournisseur ?')) {
      this.store.dispatch(ErpActions.removeFactureFournisseur({ id }));
    }
  }
