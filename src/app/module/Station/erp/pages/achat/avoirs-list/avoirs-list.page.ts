...
export class AchatAvoirsListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(ErpActions.loadAvoirsFournisseur());
  }
  ...
  onSaved(draft: AvoirFournisseurDraft): void {
    ...
    if (editing) {
      this.store.dispatch(ErpActions.updateAvoirFournisseur({ id: editing.id, draft }));
    } else {
      this.store.dispatch(ErpActions.addAvoirFournisseur({ draft }));
    }
  ...
  removeAvoir(id: number): void {
    if (confirm('Supprimer cet avoir fournisseur ?')) {
      this.store.dispatch(ErpActions.removeAvoirFournisseur({ id }));
    }
  }
