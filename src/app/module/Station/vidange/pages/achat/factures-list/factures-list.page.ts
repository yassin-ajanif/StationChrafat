...
export class AchatFacturesListPage implements OnInit {
  ...
  onSaved(draft: FactureFournisseurDraft): void {
    ...
    if (editing) {
      this.store.dispatch(VidangeActions.updateFactureFournisseur({ id: editing.id, draft }));
    } else {
      this.store.dispatch(VidangeActions.addFactureFournisseur({ draft }));
    }
  ...
  removeFacture(id: number): void {
    if (confirm('Supprimer cette facture fournisseur ?')) {
      this.store.dispatch(VidangeActions.removeFactureFournisseur({ id }));
    }
  }
