...
export class DevisListPage implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(VidangeActions.loadDevis());
  }
  ...
  onSaved(draft: DevisDraft): void {
    ...
    if (editing) {
      this.store.dispatch(VidangeActions.updateDevis({ id: editing.id, draft }));
    } else {
      this.store.dispatch(VidangeActions.addDevis({ draft }));
    }
  ...
  removeDevis(id: number): void {
    if (confirm('Supprimer ce devis ?')) {
      this.store.dispatch(VidangeActions.removeDevis({ id }));
    }
  }
